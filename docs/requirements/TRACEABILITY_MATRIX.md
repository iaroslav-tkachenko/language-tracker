...
This matrix connects business intent to functional behavior, end-to-end use
cases, and verification. It supports impact analysis, scope control, design
review, and MVP acceptance.

Requirement IDs are stable references. Test IDs describe the verification suite
and group executable or manual checks by purpose.

## 2. Verification identifiers
...
| `UT-STAT`           | Totals, averages, activity comparisons, future-date handling, and streak algorithms                | Unit                                      |
| `UT-ACTIVITY-DONUT` | Activity ordering, six-item boundary, `Other` grouping, and duration formatting                    | Unit                                      |
...
| `UT-CEFR`           | Reference models, eligible progress, seven-/thirty-day pace, rounding, and unavailable states      | Unit                                      |
| `DB-CONSTRAINT`     | Names, limits, durations, absence of comment fields, ownership references, and archival invariants | pgTAP                                     |
...
| `E2E-STAT`          | Board scoping, totals, averages, streaks, activity analysis, guidance, and distributions           | Playwright                                |
...
| `E2E-THEME`         | System, light, and dark selection, persistence, and system-preference changes                      | Playwright and manual review              |
...
| `BR-002` — Separate language-board history     | `FR-BOARD-001`–`FR-BOARD-008`, `FR-STAT-001`, `FR-VOCAB-001`–`FR-VOCAB-012`, `FR-CEFR-001`–`FR-CEFR-009` | `UC-03`, `UC-07`, `UC-08`, `UC-11`, `UC-12` | `E2E-BOARD`, `E2E-STAT`, `E2E-VOCAB`, `E2E-CEFR`      |
| `BR-003` — Minimize repeated logging input     | `FR-ENTRY-003`–`FR-ENTRY-005`, `FR-ENTRY-013`, `FR-ENTRY-014`, `FR-ENTRY-018`–`FR-ENTRY-023`             | `UC-05`, `UC-10`                            | `E2E-ENTRY`, `UT-BATCH`, `DB-BATCH`                   |
...
| `BR-007` — Board statistics                    | `FR-STAT-001`–`FR-STAT-017`                                                                              | `UC-08`                                     | `UT-STAT`, `DB-AGG`, `E2E-STAT`                       |
| `BR-008` — Desktop and mobile browser use      | `FR-HEAT-008`, `FR-UI-001`, `FR-UI-006`, `FR-UI-008`, `FR-UI-010`, `FR-VOCAB-010`                        | `UC-05`–`UC-12`                             | `E2E-RESP`, `E2E-THEME`, `A11Y`                       |
...
| `BR-011` — Above-the-fold daily action         | `FR-ENTRY-013`–`FR-ENTRY-017`, `FR-ENTRY-023`–`FR-ENTRY-026`, `FR-UI-007`–`FR-UI-016`                    | `UC-05`, `UC-06`                            | `E2E-ENTRY`, `E2E-RESP`, `A11Y`                       |
| `BR-012` — Safe repeated date-range entry      | `FR-ENTRY-018`–`FR-ENTRY-022`                                                                            | `UC-10`                                     | `UT-BATCH`, `DB-BATCH`, `E2E-ENTRY`                   |
| `BR-013` — Separate Vocabulary tracker         | `FR-VOCAB-001`–`FR-VOCAB-012`                                                                            | `UC-11`                                     | `UT-VOCAB`, `DB-CONSTRAINT`, `DB-RLS`, `E2E-VOCAB`    |
| `BR-014` — CEFR history and qualified forecast | `FR-CEFR-001`–`FR-CEFR-009`                                                                              | `UC-12`                                     | `UT-CEFR`, `DB-CONSTRAINT`, `DB-RLS`, `E2E-CEFR`      |
| `BR-015` — Recent and reference analytics      | `FR-STAT-012`–`FR-STAT-017`                                                                              | `UC-08`, `UC-12`                            | `UT-STAT`, `UT-CEFR`, `UT-ACTIVITY-DONUT`, `E2E-STAT` |
...
| Estimate transparency                 | `NFR-TRUST-001`–`NFR-TRUST-004`                               | Self-declared CEFR language, versioned models, visible methodology and limitations                       | `UT-CEFR`, `E2E-CEFR`, content and scope review                     |
...
| No free-form comments            | `FR-ENTRY-006`, `FR-CEFR-002`, `BR-RULE-034`                             | Study, edit, batch, and CEFR forms expose no comment field; corresponding database tables contain no comment column.                  |
...
| Activity allocation grouping     | `FR-STAT-004`, `FR-STAT-012`                                             | 1–6 activities remain separate; with 7+, top five remain separate and sixth onward form expandable `Other`.                           |
| Activity-average comparison      | `FR-STAT-017`                                                            | Current/completed/future year denominators; 7/30-day periods; top-five union; `Other`; zero baselines; rounding.                      |
...
| CEFR declaration history         | `FR-CEFR-001`–`FR-CEFR-004`                                              | A0–C2 accepted; invalid/future rejected; earlier declarations remain; occupied dates reject; system never auto-promotes.              |
| CEFR forecast                    | `FR-CEFR-005`–`FR-CEFR-010`                                              | Every transition/midpoint, effective-date subtraction, 7/30 zero-inclusive days, rounding, zero pace, completed target, and C2 pass.  |
| Reference model governance       | `FR-CEFR-010`, `FR-STAT-015`, `NFR-TRUST-004`                            | Approved vocabulary midpoints and weekly learning mixes remain immutable, versioned, and identified as approximate guidance.          |

## 6. Change-impact rule

When a requirement changes, reviewers must update every affected row in this
matrix, its use case and acceptance criteria, the architecture when technical
boundaries change, and the applicable tests. An MVP item is accepted only when
its linked business requirement, functional behavior, and applicable
non-functional controls have evidence of verification.
