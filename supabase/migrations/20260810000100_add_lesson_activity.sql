...
where not exists (
  ...
  from public.activity_types activity
  where activity.user_id = profile.user_id
    and activity.system_key = 'lesson'
)
and not exists (
  select 1
  from public.activity_types activity
  where activity.user_id = profile.user_id
    and activity.archived_at is null
    and lower(activity.name) = 'lesson'
)
and (
  select count(*)
  from public.activity_types activity
